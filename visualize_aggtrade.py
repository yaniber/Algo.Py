...
def visualize_trades(df, window_size=20, speed=0.5):
    """
    Visualize trades dynamically at runtime with proper zooming.
    ...
    for i in range(len(df)):
        ...
        # Dynamically adjust the x-axis and y-axis limits to the current window
        ax.set_xlim(current_data['a'].min() - 1, current_data['a'].max() + 1)
        price_min, price_max = current_data['p'].min(), current_data['p'].max()
        ax.set_ylim(price_min - (price_max - price_min) * 0.1, price_max + (price_max - price_min) * 0.1)
