...
def insert_technical_indicators():
    """
    Inserts 200 million rows into the 'technical_indicators' table and measures performance.
    """
    conn = sqlite3.connect(DATABASE_FILE)
    cursor = conn.cursor()
                ...
                for _ in range(num_rows_per_combination):
                    ...
                    if len(data_batch) >= batch_size:
                        ...
                        if inserted_rows % 1000000 == 0:
                            ...
                            print(f"Inserted {inserted_rows}/{total_rows} rows into 'technical_indicators' in {elapsed_time:.2f} seconds (Data Gen: {total_gen_time:.2f}s, Insert: {total_insert_time:.2f}s)")
    ...
    conn.commit()
    total_time = time.time() - start_time
    print(f"Inserted total {inserted_rows} rows into 'technical_indicators' in {total_time:.2f} seconds (Data Gen: {total_gen_time:.2f}s, Insert: {total_insert_time:.2f}s)")
    conn.close()
