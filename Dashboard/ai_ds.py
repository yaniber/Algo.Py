...
- For analysis: Generate Python code in ```python blocks
- Always verify orders via user confirmation
...
class AgentEnvironment:
    ...
    def execute_code(self, code, context=""):
        ...
        try:
            ...
            ai_prompt = f"Analyze the following output and generate a user-friendly response:\n\n{result}"
...
def adjust_quantity(symbol, amount):
    """Ensure minimum notional requirements"""
    try:
    ...
    if prompt := st.chat_input("Enter trading command:"):
        ...
        try:
            ...
            # Handle JSON actions
                ...
                if 'quantity' in action and 'symbol' in action:
                    adjusted = adjust_quantity(action['symbol'], action['quantity'])
                    if adjusted != action['quantity']:
                        action['quantity'] = adjusted
                        st.session_state.messages.append({
                            "role": "system",
                            "content": f"Adjusted quantity to {adjusted} for minimum notional"
                        })
    ...
    if st.session_state.pending_code:
        ...
        if st.button("✅ Execute Code"):
            ...
            if result['success']:
                ...
                retries_left = st.session_state.get('code_retries', 3) - 1
                if retries_left > 0:
                    ...
                    st.session_state.code_retries = 3  # Reset retries
